...
export const getAllAssignments = (): Promise<ApiResponse<AssignmentResponse[]>> => {
  ...
  return request.get<ApiResponse<AssignmentResponse[]>>('/api/assignments').then(response => response.data)
}
...
export const getAssignmentsByTeacherId = (teacherId: number): Promise<ApiResponse<AssignmentResponse[]>> => {
  ...
  return request.get<ApiResponse<AssignmentResponse[]>>(`/api/assignments/teacher/${teacherId}`).then(response => response.data)
}
...
export const getAssignmentsByStudent = (): Promise<ApiResponse<AssignmentResponse[]>> => {
  ...
  return request.get<ApiResponse<AssignmentResponse[]>>('/api/assignments/student').then(response => response.data)
}
...
export const getAssignmentDetail = (id: number): Promise<ApiResponse<AssignmentResponse>> => {
  ...
  return request.get<ApiResponse<AssignmentResponse>>(`/api/assignments/${id}`).then(response => response.data)
}
...
export const createAssignment = (data: AssignmentCreateRequest): Promise<ApiResponse<AssignmentResponse>> => {
  ...
  return request.post<ApiResponse<AssignmentResponse>>('/api/assignments', data).then(response => response.data)
}
...
export const updateAssignment = (id: number, data: AssignmentCreateRequest): Promise<ApiResponse<AssignmentResponse>> => {
  ...
  return request.put<ApiResponse<AssignmentResponse>>(`/api/assignments/${id}`, data).then(response => response.data)
}

// 删除作业
export const deleteAssignment = (id: number): Promise<ApiResponse<void>> => {
  console.log(`调用deleteAssignment API, ID: ${id}`)
  return request.delete<ApiResponse<void>>(`/api/assignments/${id}`).then(response => response.data)
}

// 获取班级列表 (用于作业发布时选择班级)
export const getClassList = (params?: any): Promise<ApiResponse<ClassItem[]>> => {
  console.log('调用getClassList API, 参数:', params);
  return request.get<ApiResponse<ClassItem[]>>('api/class/list', { params }).then(response => response.data);
} 