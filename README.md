...
## 项目简介
一个基于 Vue3 + Element Plus + TypeScript 开发的 **可运行于 Web 或打包为桌面应用** 的学员/员工信息管理系统。它不仅包含完整的**用户、部门、员工、班级、学生、考试、成绩**等核心模块的增删ag改查功能，还实现了一系列高阶特性，例如：**多维度的成绩统计与排名分析、支持富文本的通知系统、学生与管理员之间的信箱交流系统、灵活的系统配置（如正则校验、日志保留策略）以及可打包为跨平台桌面应用的能力**，旨在提供一个功能全面、体验优秀的一体化管理解决方案。
